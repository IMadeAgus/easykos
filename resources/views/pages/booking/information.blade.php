            ...
            <div class="flex gap-4">
                ...
                <div class="flex flex-col gap-3 w-full">
                    ...
                    <div class="flex items-center gap-[6px]">
                        <img src="{{ asset('assets/images/icons/profile-2user.svg') }}" class="w-5 h-5 flex shrink-0"
                            alt="icon">
                        <p class="text-sm text-ngekos-grey">{{ $room->capasity }} People</p>
                    </div>
                    <div class="flex items-center gap-[6px]">
    ...
    <form action="{{ route('booking.information.save', $boardingHouse->slug) }}"
        ...
        <div id="InputContainer" class="flex flex-col gap-[18px]">
            <div class="flex flex-col w-full gap-2 px-5">
                ...
                <label
                    class="flex items-center w-full rounded-full p-[14px_20px] gap-3 bg-white focus-within:ring-1 focus-within:ring-[#91BF77] transition-all duration-300  {{ $errors->has('name') ? 'border border-red-500' : '' }}">
                    <img src="{{ asset('assets/images/icons/profile-2user.svg') }}" class="w-5 h-5 flex shrink-0"
                        alt="icon">
                    <input type="text" name="name" id=""
                        class="appearance-none outline-none w-full font-semibold placeholder:text-ngekos-grey placeholder:font-normal"
                        placeholder="Write your name" value="{{ old('phone') }}">
                </label>
                @error('name')
            ...
            <div class="flex flex-col w-full gap-2 px-5">
                ...
                <label
                    class="flex items-center w-full rounded-full p-[14px_20px] gap-3 bg-white focus-within:ring-1 focus-within:ring-[#91BF77] transition-all duration-300 {{ $errors->has('email') ? 'border border-red-500' : '' }}">
            ...
            <div class="flex flex-col w-full gap-2 px-5">
                ...
                <label
                    class="flex items-center w-full rounded-full p-[14px_20px] gap-3 bg-white focus-within:ring-1 focus-within:ring-[#91BF77] transition-all duration-300 {{ $errors->has('phone') ? 'border border-red-500' : '' }}">
